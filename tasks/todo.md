...
## 本流 (farouqaldori/vibe-notch) からの移植候補

- [ ] チャット内インライン画像表示（46d9d0f）— 送った画像を後から見返せる。ユーザー関心高
- [ ] トークントラッキング + 状態フォールバック（848562e）
- [ ] Claude configディレクトリの統一解決 + フォルダピッカー（baed4e0ほか）
- [ ] 新hookイベント登録（c567537 / ce6587b、インストール済みCCが対応するものだけ登録する方式ごと）
- [x] セッション定期再チェック → 独自実装済み（pid生存確認によるreap + 10分phase復元）
